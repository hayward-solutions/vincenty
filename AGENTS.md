...
# Build (command line)
cd clients/ios && xcodebuild -project Vincenty.xcodeproj -scheme Vincenty -sdk iphonesimulator build

# Run tests (XCTest — stubs only currently)
cd clients/ios && xcodebuild -project Vincenty.xcodeproj -scheme Vincenty -sdk iphonesimulator test
```

Open `clients/ios/Vincenty.xcodeproj` in Xcode for day-to-day development. The
...
- XcodeGen `project.yml` is the source of truth — never hand-edit `Vincenty.xcodeproj`.
...
### Testing
- XCTest targets exist (`VincentyTests`, `VincentyUITests`) but contain **stubs only** — no real tests yet.
